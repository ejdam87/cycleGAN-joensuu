...
# For discriminator models (Dx, Dy), we are going to use PatchGAN architecture

class PatchGAN(torch.nn.Sequential):
    """
    ...
    def __init__(self, in_channels, ndf=64, n_layers=3, norm_layer=torch.nn.BatchNorm2d):
        ...
        kw = 4
        padw = 1
        sequence = [torch.nn.Conv2d(in_channels, ndf, kernel_size=kw, stride=2, padding=padw), torch.nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
